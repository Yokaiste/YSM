...
// `(Name, id)` pairs inside the DeckSerializer maps. Safe to share across calls:
// String.matchAll clones the regex rather than advancing this one's lastIndex.
const DIVISION_ID_PATTERN = /\((Descriptor_Deck_Division_[A-Za-z0-9_]+),\s*(\d+)\)/g;
const UNIT_ID_PATTERN = /\(\$\/GFX\/Unit\/([A-Za-z0-9_]+),\s*(\d+)\)/g;

function upsertRenderedBlock(
  ndf: NdfReaders,
  content: string,
  blockOwnerId: string,
  render: () => string,
): string {
  return ndf.upsertGeneratedBlock(content, render(), blockOwnerId);
}
...
export function renderDivisionRulesOutput(
  ...
  return upsertRenderedBlock(ndf, content, blockOwnerId, () =>
    renderDivisionRulesBlock(ndf, divisions, blockOwnerId, scriptSourcePath),
  );
}
...
export function renderDeckPacksOutput(
  ...
  return upsertRenderedBlock(ndf, content, blockOwnerId, () =>
    renderGeneratedPacksBlock(ndf, packs, blockOwnerId, scriptSourcePath),
  );
}
...
export function renderDecksOutput(
  ...
  return upsertRenderedBlock(ndf, content, blockOwnerId, () =>
    renderDecksBlock(ndf, divisions, blockOwnerId, scriptSourcePath),
  );
}
...
export function injectDeckSerializerEntries(
  ...
  const allDivisionEntries = parseSerializerEntries(content, DIVISION_ID_PATTERN, 'division');
  const baseDivisionEntries = parseSerializerEntries(
    mergeBaseContent,
    DIVISION_ID_PATTERN,
    'division',
  );
  const allUnitEntries = parseSerializerEntries(content, UNIT_ID_PATTERN, 'unit');
  const baseUnitEntries = parseSerializerEntries(mergeBaseContent, UNIT_ID_PATTERN, 'unit');
...
function injectMapEntries(
  ...
  const body = match[2] ?? '';
  const end = match[3] ?? '';
  const blockId = buildDeckSerializerBlockId(blockOwnerId, mapName);
...
function buildDeckSerializerBlockId(blockOwnerId: string, mapName: string): string {
  return `${blockOwnerId} | DeckSerializer:${mapName}`;
}
