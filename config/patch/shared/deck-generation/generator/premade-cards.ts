...
export interface PremadeCardBuildContext {
  mode: DivisionMode;
...
export interface PremadeCardNormalizationContext extends PremadeCardBuildContext {
  ruleEntries: GeneratedRuleEntry[];
}

export function buildSelectionCards(
  selection: PremadeSelection,
  categoryKey: string,
  analysisByName: Map<string, EntityAnalysis>,
  context: PremadeCardBuildContext,
): PremadeCard[] {
  const { mode, entityByName, generatedPacks, modTag, contextCode, config } = context;
...
export function buildForcedCards(
...
function buildEntryCards(args: {
  ...
  return transportNames.flatMap((transportName) => {
    const packDescriptorName = ensureCardPackDescriptor(entity, transportName, profile, args);
...
export function normalizePremadeCards(
  cards: PremadeCard[],
  context: PremadeCardNormalizationContext,
): PremadeCard[] {
  const { ruleEntries, entityByName, config } = context;
  ...
  const normalizedCards = cards.flatMap((card): PremadeCard[] => {
    ...
    const transportName = normalizeTransportName({ card, entry, entityByName, config });
    const packDescriptorName = ensureCardPackDescriptor(
      card.entity,
      transportName,
      profile,
      context,
...
function ensureCardPackDescriptor(
  entity: EntityData,
  transportName: string | undefined,
  profile: NonNullable<ReturnType<typeof resolvePackProfile>>,
  context: PremadeCardBuildContext,
): string {
  const { mode, generatedPacks, modTag, contextCode } = context;
  return ensureGeneratedPackDescriptor({
    entity,
    mode,
    generatedPacks,
    modTag,
...
function findLowestPriorityDuplicateUnitCard(
  ...
  for (const card of cards) {
    const transport = resolveCardTransport(card, entityByName);
    if (transport) {
      transportGroupCounts[deriveTransportGroup(transport)] += 1;
    }
  }
  return findLowestScoringCardIndex(cards, (card) => {
...
function findLowestPriorityRemovableCard(args: {
  ...
  return findLowestScoringCardIndex(cards, (card) => {
    ...
    return resolveRemovalScore(card, cards, entityByName, true, scoreContext);
  });
}

function findLowestScoringCardIndex(
  cards: readonly PremadeCard[],
  scoreCard: (card: PremadeCard) => number | undefined,
): number {
  let removeIndex = -1;
...
function resolveCoverageFloor(
  ...
  // Greedy set cover: each round takes the entity covering the most wanted roles.
  let uncoveredRoles = 0;
  for (const remaining of remainingByRole.values()) {
    if (remaining > 0) uncoveredRoles += 1;
  }

  let selectedCount = 0;
  while (uncoveredRoles > 0) {
    let bestEntityName: string | undefined;
    let bestContribution = 0;
    for (const entityName of availableEntities) {
      let contribution = 0;
      for (const roleKey of rolesByEntity.get(entityName) ?? []) {
        if ((remainingByRole.get(roleKey) ?? 0) > 0) contribution += 1;
      }
      if (
        contribution > bestContribution ||
        (contribution === bestContribution &&
          bestEntityName !== undefined &&
          entityName.localeCompare(bestEntityName) < 0)
      ) {
        bestEntityName = entityName;
        bestContribution = contribution;
      }
    }
    if (bestEntityName === undefined || bestContribution <= 0) {
      break;
    }

    availableEntities.delete(bestEntityName);
    selectedCount += 1;
    for (const roleKey of rolesByEntity.get(bestEntityName) ?? []) {
      const remaining = remainingByRole.get(roleKey) ?? 0;
      if (remaining > 0) {
        remainingByRole.set(roleKey, remaining - 1);
        if (remaining === 1) uncoveredRoles -= 1;
      }
    }
  }
...
/** The transport entity a card carries, or undefined when it rides none. */
function resolveCardTransport(
  card: Pick<PremadeCard, 'transportName'>,
  entityByName: ReadonlyMap<string, EntityData>,
): EntityData | undefined {
  return card.transportName === undefined ? undefined : entityByName.get(card.transportName);
}
...
function resolveRemovalScore(
  ...
  const transportEntity = resolveCardTransport(card, entityByName);
  const isAirTransport =
    transportEntity !== undefined && deriveTransportGroup(transportEntity) === 'Air';
  ...
  const transportIsSellable = hasSameUnitVariant && transportEntity?.isSellable === true;
