...
export function trimCategoryCards(args: {
  ...
  while (selected.length > limit) {
    const duplicateIndex = findLowestPriorityDuplicateUnitCard(selected, entityByName);
    if (duplicateIndex < 0) {
      break;
    }
      ...
      .map((unitName) => ({
        ...
        score: Math.max(
          ...selected
            ...
            .map((card) => resolveRemovalScore(card, selected, entityByName, false)),
...
function findLowestPriorityDuplicateUnitCard(
  ...
  for (const [index, card] of cards.entries()) {
    ...
    const overrepresentationPenalty = group
      ...
      : 0;
    const score = resolveRemovalScore(card, cards, entityByName, false) - overrepresentationPenalty;
...
function findLowestPriorityRemovableCard(args: {
  ...
  let removalScore = Number.POSITIVE_INFINITY;
  for (const [index, card] of cards.entries()) {
    if (card.forcedInPremade || !canRemoveCardWithoutLosingCoverage(card, cards, coverageTargets)) {
      continue;
    }
    const score = resolveRemovalScore(card, cards, entityByName);
    if (score < removalScore) {
      removeIndex = index;
      removalScore = score;
    }
...
function canRemoveCardWithoutLosingCoverage(
  ...
  for (const roleKey of removedCard.roleKeys) {
    ...
    const remainingEntities = new Set(
      cards
        .filter((card) => card !== removedCard && cardProvidesRoleKey(card, roleKey))
        .map((card) => card.entity.name),
    );
    if (remainingEntities.size < requiredCount) {
      return false;
    }
  }
  return true;
...
function buildRoleCoverageTargets(cards: PremadeCard[], limit: number): Map<string, number> {
  ...
  const rolesByPriority = [...entitiesByRole.keys()].sort(
    (left, right) =>
      resolveCoverageRolePriority(right, cards) - resolveCoverageRolePriority(left, cards) ||
  ...
  for (let requestedCount = 1; requestedCount <= 3; requestedCount += 1) {
    for (const roleKey of rolesByPriority) {
      ...
      if (resolveCoverageFloor(cards, candidateTargets) <= limit) {
...
function resolveCoverageFloor(
  ...
  for (const card of cards) {
    const relevantRoles = card.roleKeys.filter((roleKey) => coverageTargets.has(roleKey));
    if (relevantRoles.length === 0) {
      continue;
    }
    const entityRoles = rolesByEntity.get(card.entity.name) ?? new Set<string>();
    for (const roleKey of relevantRoles) {
      entityRoles.add(roleKey);
    }
    rolesByEntity.set(card.entity.name, entityRoles);
  }
...
function findLowestPriorityCoveredRole(
  ...
  return reducibleEntries
    ...
    .sort(
      (left, right) =>
        resolveCoverageRolePriority(left, cards) - resolveCoverageRolePriority(right, cards) ||
        left.localeCompare(right),
    )[0];
}

function resolveCoverageRolePriority(roleKey: string, cards: PremadeCard[]): number {
  return (
    Math.max(
      ...cards.filter((card) => card.roleKeys.includes(roleKey)).map((card) => card.keepPriority),
      0,
    ) + (isCommandCoverageRoleKey(roleKey) ? 350 : 0)
  );
}
...
function pruneSimilarityCards(args: {
  ...
  while (true) {
      ...
      const nearDuplicates = selected
        ...
        .filter(
          ({ card, index }) =>
            ...
            !card.forcedInPremade &&
            card.entity.name !== candidate.entity.name &&
            areNearDuplicateProfiles(candidate, card),
        )
        .sort((left, right) => comparePremadeCardPriority(left.card, right.card, context, config));
      const representative = nearDuplicates[0];
      if (
        !representative ||
        comparePremadeCardPriority(candidate, representative.card, context, config) < 0
      ) {
        continue;
      }
      const mergedRoleKeys = [...new Set([...representative.card.roleKeys, ...candidate.roleKeys])];
      consolidation = {
    ...
    const representative = selected[consolidation.representativeIndex];
    if (representative) {
...
function resolveRemovalScore(
  ...
  const similarityCount = selected.filter(
    (candidate) => candidate.similarityKey === card.similarityKey,
  ).length;
  const sameUnitGroundVariantExists =
    Boolean(card.transportName) &&
    selected.some((candidate) => {
      if (
        candidate.entity.name !== card.entity.name ||
        candidate.transportName === card.transportName
      ) {
        return false;
      }
      const transportEntity = entityByName.get(candidate.transportName ?? '');
      return transportEntity && deriveTransportGroup(transportEntity) === 'Ground';
    });
  ...
  const hasSameUnitVariant = selected.some(
    (candidate) => candidate !== card && candidate.entity.name === card.entity.name,
  );
  ...
  const roleCoverageProtection = card.roleKeys.reduce((total, roleKey) => {
    const coverageCount = new Set(
      selected
        .filter((candidate) => cardProvidesRoleKey(candidate, roleKey))
        .map((candidate) => candidate.entity.name),
    ).size;
