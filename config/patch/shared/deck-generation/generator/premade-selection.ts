...
function isUsefulEconomyOption(args: {
  ...
  // An economy pick must save meaningfully against every stronger selection; a real
  // weapon niche can still win its own price-independent role.
...
function tokenSimilarity(left: readonly string[], right: readonly string[]): number {
  return resolveSetJaccard(toCachedTokenSet(left), toCachedTokenSet(right));
}

const DIVERSITY_PROFILE_PREFIXES = [...WEAPON_PROFILE_TOKEN_PREFIXES, 'scope:'] as const;
...
function resolveMedianCost(candidates: Array<{ analysis: EntityAnalysis }>): number {
  return resolveMedian(candidates.map((candidate) => candidate.analysis.entity.cost));
}
