...
import {
  type AmmunitionData,
  type Coalition,
  type DivisionCostMatrix,
  type DivisionDescriptorData,
  type DivisionMemberEntry,
...
function readTagArrayValue(value: string | undefined, ndf: NdfReaders): string[] {
  return readListValues(value, ndf)
    .map((entry) =>
      String(entry)
...
function readListValues(value: string | undefined, ndf: NdfReaders): unknown[] {
  if (!value) return [];
  const trimmed = value.trim();
  return trimmed.startsWith('[') && trimmed.endsWith(']')
    ? ndf.parseList(trimmed).map((entry) => entry.value)
    : [];
}
...
/** `exactOptionalPropertyTypes` rejects explicit `undefined`, so absent means no key. */
...
interface UnitRuleEntry {
  text: string;
  unitName: string;
  transportNames: string[];
}

/** Walks a `UnitRuleList` value and yields every resolvable `TDeckUniteRule` entry. */
function* readUnitRuleEntries(
  unitRuleListValue: string,
  ndf: NdfReaders,
): Generator<UnitRuleEntry> {
  for (const entry of ndf.findCollectionEntries(unitRuleListValue)) {
    ...
    yield {
      text: entry.text,
      unitName,
      transportNames: extractUnitDescriptorNames(
        ndf.readPath(entry.text, ['AvailableTransportList']),
        ndf,
      ),
    };
  }
...
function readDivisionRuleData(entry: UnitRuleEntry, ndf: NdfReaders): DivisionRuleData {
...
function addRuleCandidate(
  candidatesByUnit: Map<string, DivisionRuleData[]>,
  rule: DivisionRuleData,
): void {
  const candidates = candidatesByUnit.get(rule.unitName) ?? [];
  candidates.push(rule);
  candidatesByUnit.set(rule.unitName, candidates);
}
...
const COALITION_REFERENCE_PATTERN = /^TWargameCoalition\/([A-Za-z0-9_]+)$/;
const MATRIX_NAME_PATTERN = /^[A-Za-z0-9_]+$/u;

/** A descriptor with an inline rule has no named one, so each is gated under `<Division>_Rule`. */
export function parseDivisionDescriptors(
  divisionRulesContent: string,
  divisionsContent: string,
  ignoredRuleNamePatterns: RegExp[],
  ndf: NdfReaders,
): DivisionDescriptorData[] {
  ...
  const blocks = collectTopLevelBlocksByType(divisionsContent, 'TDeckDivisionDescriptor', ndf);
  // A decooked division keeps its coalition only as a DivisionTags token, and
  // readTagArrayValue lowercases tags.
  ...
  const descriptors: DivisionDescriptorData[] = [];
  for (const block of blocks) {
    ...
    const coalition =
      readReferenceSuffix(ndf.readField(body, 'DivisionCoalition'), COALITION_REFERENCE_PATTERN) ??
    ...
    const maxActivationPoints = readInteger(ndf.readField(body, 'MaxActivationPoints'));
    const costMatrixValue = ndf.readField(body, 'CostMatrix');
    const costMatrixName = costMatrixValue?.trim().match(MATRIX_NAME_PATTERN)?.[0];
    const divisionRuleValue = ndf.readField(body, 'DivisionRule');
    const inlineUnitRuleListValue = divisionRuleValue
      ? ndf.readField(divisionRuleValue, 'UnitRuleList')
      : undefined;
    const unitRuleListValue =
      inlineUnitRuleListValue ??
      (divisionRuleValue ? unitRuleListByRuleName.get(divisionRuleValue.trim()) : undefined);
    const members: DivisionMemberEntry[] = [];
    const inlineUnitRules: DivisionRuleData[] = [];
    for (const entry of readUnitRuleEntries(unitRuleListValue ?? '', ndf)) {
      members.push({ unitName: entry.unitName, transportNames: entry.transportNames });
      if (inlineUnitRuleListValue) {
        inlineUnitRules.push(readDivisionRuleData(entry, ndf));
      }
    }
    ...
    descriptors.push({
      name: block.name,
      ...(coalition ? { coalition } : {}),
      ...(maxActivationPoints === undefined ? {} : { maxActivationPoints }),
      ...(costMatrixName
        ? { costMatrixName }
        : { inlineCostMatrix: readCostMatrix(costMatrixValue, ndf) }),
      members,
      inlineUnitRules,
    });
  }
...
/** Every named `MatrixCostName_* is MAP [...]` block. */
export function parseDivisionCostMatrices(
  content: string,
  ndf: NdfReaders,
): Map<string, DivisionCostMatrix> {
  return new Map(
    collectTopLevelBlocksByType(content, 'MAP', ndf).flatMap((block) =>
      block.name ? [[block.name, readCostMatrix(block.text, ndf)] as const] : [],
    ),
  );
}

/** Reads a `MAP [(Factory/X, [...]), ...]` value into its per-category rows. */
function readCostMatrix(value: string | undefined, ndf: NdfReaders): DivisionCostMatrix {
  const rows = new Map<string, number[]>();
  if (!value) {
    return rows;
  }
  for (const entry of ndf.findCollectionEntries(value)) {
    const factoryType = readReferenceSuffix(entry.text, /Factory\/([A-Za-z0-9_]+)/);
    if (factoryType) {
      rows.set(factoryType, normalizeNumberArrayValue(entry.text.match(/\[[^[\]]*\]/u)?.[0], ndf));
    }
  }
  return rows;
}

/** Inline rules arrive through the descriptors, named ones from the top-level scan. */
export function parseDivisionRuleData(
  ...
  for (const block of collectTopLevelBlocksByType(divisionRulesContent, 'TDeckDivisionRule', ndf)) {
    ...
    for (const entry of readUnitRuleEntries(unitRuleListValue, ndf)) {
      addRuleCandidate(candidatesByUnit, readDivisionRuleData(entry, ndf));
    }
  }

  for (const descriptor of descriptors) {
    for (const rule of descriptor.inlineUnitRules) {
      addRuleCandidate(candidatesByUnit, rule);
    }
  }
...
function extractUnitDescriptorNames(value: string | undefined, ndf: NdfReaders): string[] {
  return readListValues(value, ndf)
    .map((entry) => extractUnitDescriptorName(String(entry)))
...
function parseAmmunitionBlock(block: NdfBlock, ndf: NdfReaders): AmmunitionData {
  ...
  return {
    ...
    ...withoutUndefined({
      ...
      accuracyMoving,
      minimumRangeGru,
      minimumRangeHelicopterGru,
      minimumRangeAirplaneGru,
      maximumRangeGru,
      ...
      isFireAndForget,
      tirIndirect,
      forceHitTopArmorOnSuccess,
      computeArmorFromImpactLocation,
      piercingWeapon,
      tandemCharge,
    }),
  };
...
/** Resolves capacity implementations, so signatures survive renamed units. */
...
export function parseDivisionCoalitionsByUnit(
  ...
  for (const descriptor of descriptors) {
    const coalition = descriptor.coalition;
    if (!coalition) {
      continue;
    }
    for (const member of descriptor.members) {
      for (const unitName of [member.unitName, ...member.transportNames]) {
