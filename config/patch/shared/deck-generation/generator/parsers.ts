...
/**
 * Drops the keys a descriptor did not declare. `exactOptionalPropertyTypes`
 * rejects an explicit `undefined` on an optional field, so an absent NDF field
 * has to become an absent key rather than an undefined one.
 */
...
function collectUnitRuleListMembers(
  ...
  for (const { text, unitName, transportNames } of readUnitRuleEntries(unitRuleListValue, ndf)) {
    ...
    const multipliers = normalizeNumberArrayValue(
      ndf.readPath(text, ['NumberOfUnitInPackXPMultiplier']),
      ndf,
    );
    const candidates = candidatesByUnit.get(unitName) ?? [];
    candidates.push({
      unitName,
      maxPackNumber: readInteger(ndf.readPath(text, ['MaxPackNumber'])) ?? 0,
      numberOfUnitInPack: readInteger(ndf.readPath(text, ['NumberOfUnitInPack'])) ?? 0,
...
function collectMemberUnitNames(unitRuleListValue: string, ndf: NdfReaders): string[] {
  return [...readUnitRuleEntries(unitRuleListValue, ndf)].flatMap(
    ({ unitName, transportNames }) => [unitName, ...transportNames],
  );
}
