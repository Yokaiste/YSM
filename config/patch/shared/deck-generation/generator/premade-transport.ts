...
export function normalizeTransportName(args: {
  ...
  if (card.selectionKind.endsWith('cheap')) {
    return pickBestTransportName({
      transportNames: entry.transportNames,
      unit: card.entity,
...
/**
 * Fallback for a transport that never entered the competitive analysis pool.
 * It mirrors {@link scoreTransportAnalysis} term for term on the shared chassis
 * metrics, so an analysed and an unanalysed transport stay comparable. Armament
 * has no weapon analysis here, so the descriptor count stands in for it at a
 * weight in line with the analysed `(antiGround + antiArmor) * 0.12` term.
 */
