...
// A weapon channel or ammunition payload below this share of the unit's strongest
// one is incidental -- a self-defense machine gun on a tank, not a role the unit
// can be selected for. Used to gate channels, payloads, and capability tokens.
...
// Platform metrics describe the chassis alone, with no weapon analysis involved.
// They are shared so that scoring a unit without a full EntityAnalysis (a
// transport outside the competitive pool) cannot drift from the real metrics.
...
/**
 * Chassis scores are pure functions of one entity, and card selection asks for
 * the same entity again for every context, mode, and sort comparison it takes
 * part in. Caching them turns the whole roster's worth of repeated arithmetic
 * and string work into one pass.
 */
 ...
 * Used both to rank weapons against each other and to decide whether a secondary
 * channel is a real role or incidental.
 */
