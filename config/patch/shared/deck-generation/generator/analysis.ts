...
// readSimilarityMetric runs inside every pairwise duplicate/dominance comparison,
// so the vector position has to be a lookup rather than a scan.
const SIMILARITY_METRIC_INDEXES = new Map<MetricKey, number>(
  SIMILARITY_METRIC_KEYS.map((metric, index) => [metric, index]),
);
...
const NON_TACTICAL_DOMINANCE_METRICS = new Set<MetricKey>([
  'amphibious',
  'availability',
  'command',
  'supply',
...
const TACTICAL_DOMINANCE_METRICS = METRIC_KEYS.filter(
  (metric) => !NON_TACTICAL_DOMINANCE_METRICS.has(metric),
);

// Below this share of the unit's strongest channel a weapon is incidental -- a
// tank's self-defense machine gun, not a role it can be selected for.
const INCIDENTAL_STRENGTH_RATIO = 0.12;

export const WEAPON_PROFILE_TOKEN_PREFIXES = [
  'ammo:',
  'ammo_loadout:',
  'channel:',
  'effect:',
...
export interface EntityAnalysisFactory {
  analyze(entry: GeneratedRuleEntry): EntityAnalysis;
}
...
export function resolveSelectionPurposeGroupLimit(profile: SelectionPurposeProfile): number {
  // A named mechanic is one exact job; broader roles tolerate more variety.
...
export function buildCategoryAnalyses(args: {
  ...
  const analysisFactory =
    args.analysisFactory ?? createEntityAnalysisFactory(args.weaponDescriptors, args.ammunition);
  const analyses = args.entries.map((entry) => analysisFactory.analyze(entry));
  const stats = buildCategoryAnalysisStats(analyses);
  for (const analysis of analyses) {
    // Free units keep the raw utility; priced ones are measured per command point.
    const utility = resolveUtilityScore(analysis.metrics);
    analysis.metrics.value = analysis.entity.cost > 0 ? utility / analysis.entity.cost : utility;
  }
...
function resolveUtilityScore(metrics: Readonly<Record<MetricKey, number>>): number {
  return (
    metrics.antiArmor +
    metrics.antiGround +
    metrics.antiHelo +
    ...
    metrics.suppression * 0.2 +
    metrics.mobility * 0.1 +
    metrics.fireOnMove * 0.18 +
    metrics.initiative * 0.12 +
    metrics.weaponVariety * 0.35
  );
}

export function createEntityAnalysisFactory(
  weaponDescriptors: Map<string, WeaponDescriptorData>,
  ammunition: Map<string, AmmunitionData>,
): EntityAnalysisFactory {
  const baseByEntity = new WeakMap<EntityData, EntityAnalysis>();
  return {
    analyze(entry: GeneratedRuleEntry): EntityAnalysis {
      let base = baseByEntity.get(entry.entity);
      if (!base) {
        base = analyzeEntry(entry, weaponDescriptors, ammunition);
        base.metrics.availability = 0;
        base.relativeMetrics.availability = 0;
        baseByEntity.set(entry.entity, base);
      }
      const metrics = {
        ...base.metrics,
        availability: resolveEntryAvailability(entry),
      };
      return {
        entry,
        entity: base.entity,
        typeKey: base.typeKey,
        similarityKey: base.similarityKey,
        similarityVector: [],
        roleTokens: [...base.roleTokens],
        exactRoleTokens: new Set(base.exactRoleTokens),
        profileTokens: [...base.profileTokens],
        metrics,
...
// Shared so scoring a unit without a full EntityAnalysis cannot drift from the
// real metrics.

export function resolveEntityAgility(entity: EntityData): number {
  return (entity.agilityRadiusGru ?? 0) > 0
    ? 100_000 / Math.max(1, entity.agilityRadiusGru ?? 1)
    : 0;
}

export function resolveEntityStealth(entity: EntityData): number {
  return (entity.concealmentBonus ?? 0) * 180;
}

export function resolveEntityAmphibiousBonus(entity: EntityData): number {
  return isAmphibiousEntity(entity) ? 180 : 0;
}

/** Memoized: the same entity is scored once per context, mode, and comparison. */
function memoizePerEntity<Value>(
  compute: (entity: EntityData) => Value,
): (entity: EntityData) => Value {
  const cache = new WeakMap<EntityData, Value>();
  return (entity) => {
    if (cache.has(entity)) {
      return cache.get(entity) as Value;
    }
    const value = compute(entity);
...
export const resolveEntitySurvivability = memoizePerEntity(
  (entity: EntityData) =>
    ...
    resolveEntityAgility(entity) * 1.5,
);

export const resolveEntityMobility = memoizePerEntity((entity: EntityData) => {
  const speed = entity.maxSpeedKmph ?? 0;
  const endurance = (entity.fuelCapacity ?? 0) * 0.18 + (entity.fuelMoveDuration ?? 0) * 0.42;
  return (
    speed * 20 +
    speed * (entity.speedBonusFactorOnRoad ?? 1) * 4 +
    endurance +
    resolveEntityAgility(entity) * 1.2 +
    resolveEntityAmphibiousBonus(entity) +
...
function analyzeEntry(
  ...
  const agility = resolveEntityAgility(entity);
  const responseTime =
    Math.max(0, 30 - Math.max(0, entity.travelDuration ?? 30)) * 32 +
    Math.max(0, 25 - Math.max(0, entity.evacuationTime ?? 25)) * 20;
  const survivability = resolveEntitySurvivability(entity);
  const mobilityBase = entity.maxSpeedKmph ?? 0;
  const amphibious = resolveEntityAmphibiousBonus(entity);
  const stealth = resolveEntityStealth(entity);
  const optics =
  ...
  const mobility = resolveEntityMobility(entity);
  const initiative =
...
function resolveEntryAvailability(entry: GeneratedRuleEntry): number {
  return (
...
export function areNearDuplicateProfiles(
  ...
  if (isCommandEntity(left.entity) && isCommandEntity(right.entity)) {
    // Same deck function; only a different deployment envelope earns another card.
    if (haveMaterialDeploymentDifference(left.entity, right.entity)) {
      return false;
    }
    if (tokenSetSimilarity(leftWeaponProfile, rightWeaponProfile) >= 0.35) {
      return true;
    }
  }
  if (
  ...
  if (
    ...
    haveEqualArrays(leftWeaponProfile, rightWeaponProfile) &&
...
const TACTICAL_QUALITY_WEIGHTS: ReadonlyArray<readonly [MetricKey, number]> = [
  ['alphaArmor', 0.55],
  ['alphaGround', 0.55],
  ['alphaHelo', 0.55],
  ['alphaPlane', 0.55],
...
function resolveTacticalProfileQuality(profile: SimilarityProfile): number {
  let total = 0;
  for (const [metric, weight] of TACTICAL_QUALITY_WEIGHTS) {
    total += readSimilarityMetric(profile, metric) * weight;
  }
  return total;
}
...
function readSimilarityMetric(profile: SimilarityProfile, metric: MetricKey): number {
  const index = SIMILARITY_METRIC_INDEXES.get(metric);
  return index === undefined ? 0 : (profile.similarityVector[index] ?? 0);
}
...
function haveMaterialSpecializationDifference(
  ...
  // Concealment and sensors make a distinct use case even under a stronger payload.
...
function haveDifferentAmmunitionLoadouts(
  ...
  // A strict payload superset is an upgrade, not a reason to keep the subset. The
  // significance cutoff is relative to each unit, so twins may straddle it.
  return (
...
/** Jaccard overlap of two sets; 0 when either side is empty. */
export function resolveSetJaccard(left: ReadonlySet<string>, right: ReadonlySet<string>): number {
...
function tokenSetSimilarity(left: readonly string[], right: readonly string[]): number {
  return resolveSetJaccard(new Set(left), new Set(right));
}
...
function haveCompatibleProfileTokens(left: readonly string[], right: readonly string[]): boolean {
  ...
  return tokenSetSimilarity(left, right) >= 0.5;
}
...
const NON_WEAPON_METRICS = new Set<MetricKey>([
  'amphibious',
  'availability',
  'command',
  'effect',
...
const WEAPON_METRIC_KEYS = METRIC_KEYS.filter(
  (metric): metric is WeaponMetricKey => !NON_WEAPON_METRICS.has(metric),
);
...
function collectWeaponStats(
    ...
    for (const mountedWeapon of descriptor.mountedWeapons) {
      ...
      tacticalPayloadStrengths.set(
        ...
        Math.max(
          ...
          resolveStrongestCombatPurpose(contribution),
        ),
      );
  ...
  const strongestBox = Math.max(...allWeaponBoxes.map(resolveStrongestCombatPurpose), 0);
  stats.ammo = allWeaponBoxes.reduce((total, box) => {
    const relevance = strongestBox > 0 ? resolveStrongestCombatPurpose(box) / strongestBox : 0;
  ...
  const meaningfulPayloadCount = [...tacticalPayloadStrengths.values()].filter(
    (strength) => strongestPayload <= 0 || strength >= strongestPayload * INCIDENTAL_STRENGTH_RATIO,
  ).length;
...
/** How strong this unit, weapon box, or payload is at whichever job it does best. */
function resolveStrongestCombatPurpose(
  metrics: Readonly<
    Record<'antiArmor' | 'antiGround' | 'antiHelo' | 'antiPlane' | 'indirect', number>
  >,
): number {
  return Math.max(
    metrics.antiArmor,
...
function resolveArmorLethality(armorPenetration: number): number {
  ...
  // Samples light to super-heavy armor so low-AP volume cannot pose as anti-tank.
...
function collectWeaponProfileTokens(
  ...
  for (const payload of payloads) {
    ...
    if (strongestPayload > 0 && payload.strength < strongestPayload * INCIDENTAL_STRENGTH_RATIO) {
      continue;
    }
  ...
  const strongestPurpose = resolveStrongestCombatPurpose(weaponStats);
  ...
  for (const [channel, score] of channelMetrics) {
    if (strongestPurpose > 0 && score < strongestPurpose * INCIDENTAL_STRENGTH_RATIO) {
...
const CANONICAL_ROLE_KIND_ORDER: Record<RoleDefinition['kind'], number> = {
  primary: 0,
  type: 1,
  role: 2,
  capability: 3,
...
function resolveCanonicalRoleKindOrder(kind: RoleDefinition['kind']): number {
  return CANONICAL_ROLE_KIND_ORDER[kind];
}
...
function resolveStrongestPurposeStrength(analysis: EntityAnalysis): number {
  return resolveStrongestCombatPurpose(analysis.metrics);
}
...
function applyCapabilityTokens(analyses: EntityAnalysis[]): void {
  for (const analysis of analyses) {
    // Profile tokens are already lower-case and fixed for this analysis, so the
    // lookup set is built once instead of once per addWeaponProfileCapability call.
    const profileTokenSet = new Set(analysis.profileTokens.map((token) => token.toLowerCase()));
    const addWeaponProfileCapability = (name: string, requiredTokens: string[]): void => {
      if (requiredTokens.some((token) => profileTokenSet.has(token))) {
    ...
    addWeaponProfileCapability('radar_guided', ['mechanic:trait_radar']);
    addWeaponProfileCapability('sead', ['mechanic:trait_sead']);
    addWeaponProfileCapability('napalm', ['mechanic:trait_napalm', 'mechanic:damage_napalm']);
    addWeaponProfileCapability('cluster', ['mechanic:trait_cluster', 'mechanic:damage_cluster']);
    addWeaponProfileCapability('close_quarters', ['mechanic:trait_cac']);
    addWeaponProfileCapability('fire_and_forget', [
      'mechanic:fire_and_forget',
      'mechanic:trait_f_f',
    ]);
    addWeaponProfileCapability('tandem', ['mechanic:tandem', 'mechanic:trait_tandem']);
    addWeaponProfileCapability('high_off_boresight', ['mechanic:trait_f_f_boresight']);
    ...
    if (analysis.metrics.precision > 0) {
      addWeaponProfileCapability('top_attack', ['mechanic:top_attack']);
    }
    ...
    for (const rangeToken of analysis.profileTokens.filter((token) =>
      token.startsWith('range_class:'),
    )) {
...
function hasMaterialCombatChannel(
  ...
  const strongestPurpose = resolveStrongestCombatPurpose(analysis.metrics);
  return strongestPurpose <= 0 || score >= strongestPurpose * INCIDENTAL_STRENGTH_RATIO;
}
...
function markStandoutCapability(
  ...
  if (scored.length > 1 && topScore - bottomScore <= Math.max(1, topScore * 0.03)) {
    // A flat metric has no standouts; unlimited cards must not get one by name order.
    return;
  }
...
function mergeRoleFocus(
  ...
  // Correlations find modded roles; a known token demotes them to supporting evidence.
...
const ROLE_KIND_PRIORITIES: Record<RoleDefinition['kind'], number> = {
  primary: 50,
  type: 40,
  role: 30,
  capability: 20,
...
function resolveRoleKindPriority(kind: RoleDefinition['kind']): number {
  return ROLE_KIND_PRIORITIES[kind];
}

const isAmphibiousEntity = memoizePerEntity(
  (entity: EntityData) =>
...
export function resolveMedian(values: number[]): number {
  if (values.length === 0) {
    return 0;
  }
