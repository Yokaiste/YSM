...
function buildPremadeCategoryCards(args: {
  ...
  const forcedEntries = entries.filter((entry) =>
    shouldForceEntityInPremade(entry.entity, context, mode, config),
  );
  const competitiveEntries = entries.filter(
    (entry) => !shouldForceEntityInPremade(entry.entity, context, mode, config),
  );
  const {
