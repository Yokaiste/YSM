...
/** Authored identifiers remain ordinary keys even when they resemble object internals. */
...
/** Element-wise equality for two ordered lists of primitives. */
export function haveEqualArrays<T>(left: readonly T[], right: readonly T[]): boolean {
...
export function derivePlatformForm(entity: EntityData): PlatformForm {
  ...
  if (entity.isTransportable && matchesPlatformKeyword(entity, INFANTRY_PLATFORM_KEYWORDS)) {
  ...
  // Tracked is the fallback, so there is nothing to test for: a ground platform
  // is wheeled if it says so anywhere, and tracked otherwise.
  if (matchesPlatformKeyword(entity, WHEELED_PLATFORM_KEYWORDS)) {
    return cachePlatformForm(entity, 'wheeled_vehicle');
  }
  return cachePlatformForm(entity, 'tracked_vehicle');
}

// WARNO descriptors mix English and French spellings for the same platform.
const INFANTRY_PLATFORM_KEYWORDS = ['infantry', 'infanterie'] as const;
const WHEELED_PLATFORM_KEYWORDS = ['wheel', 'roue'] as const;

/** True when any movement field or tag mentions one of these words. */
function matchesPlatformKeyword(entity: EntityData, keywords: readonly string[]): boolean {
...
/**
 * NDF keeps XP multipliers as float32, so an authored 0.7 arrives as 0.699999988
 * and 5 x it lands just under 3.5. Rounding that down gives a pack count the game
 * did not compute, and it rejects the whole deck.
 */
