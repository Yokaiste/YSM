...
export interface DeckGenerationInput {
  ...
  modTag: string;
  /** The script that produced the output, named in every generated block header. */
  ...
  divisionCostMatrixContent: string;
...
/** Row length is how many cards the category allows, so an empty row fields none. */
export type DivisionCostMatrix = ReadonlyMap<string, number[]>;

/** One card a division offers, and the transports that card may be taken with. */
export interface DivisionMemberEntry {
  /** The unit holding the card, which is what spends a slot in its category. */
  unitName: string;
  /** Transports riding on that card. They spend no slot of their own. */
  transportNames: string[];
}

/** A decooked division writes its coalition as a tag and its rule and matrix inline. */
export interface DivisionDescriptorData {
  name: string;
  coalition?: Coalition;
  maxActivationPoints?: number;
  /** Names a matrix in `DivisionCostMatrix.ndf`; absent when written inline. */
  costMatrixName?: string;
  /** The matrix of an inline `CostMatrix = MAP [...]`; absent when named. */
  inlineCostMatrix?: DivisionCostMatrix;
  members: DivisionMemberEntry[];
  /** Availability rules a division carries inline instead of naming one. */
  inlineUnitRules: DivisionRuleData[];
}
...
export interface DivisionContext {
  ...
  /** Overrides the per-side, per-mode emblem this context would otherwise use. */
