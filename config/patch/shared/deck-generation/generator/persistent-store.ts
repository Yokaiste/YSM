...
const DIVISION_TOKEN_PREFIX = 'YD';
const DECK_TOKEN_PREFIX = 'YK';
const MAX_TOKEN_NUMBER = 99_999_999;
const GENERATED_TOKEN_PATTERN = new RegExp(
  `^(?:${DIVISION_TOKEN_PREFIX}|${DECK_TOKEN_PREFIX})\\d{8}$`,
);

/** Tokens this store mints, so generated rows can be told from hand-written ones. */
export function isGeneratedIdentityToken(token: string): boolean {
  return GENERATED_TOKEN_PATTERN.test(token);
}

export function collectStoredIdentityTokens(store: PersistentStore): string[] {
  return Object.values(store.divisions).flatMap((metadata) => [
    metadata.divisionNameToken,
    metadata.deckNameToken,
  ]);
}
...
export function ensurePersistentDivisionMetadata(
  ...
  if (!metadata) {
    metadata = {
      ...
      divisionNameToken: generateUniqueToken(localisationState, () => {
        assertTokenNumberAvailable(store.nextDivisionNameToken, 'division-name');
        const token = `${DIVISION_TOKEN_PREFIX}${String(store.nextDivisionNameToken).padStart(8, '0')}`;
        store.nextDivisionNameToken += 1;
        return token;
      }),
      deckNameToken: generateUniqueToken(localisationState, () => {
        assertTokenNumberAvailable(store.nextDeckNameToken, 'deck-name');
        const token = `${DECK_TOKEN_PREFIX}${String(store.nextDeckNameToken).padStart(8, '0')}`;
  ...
  upsertLocalisationRow(localisationState, metadata.divisionNameToken, divisionName);
  upsertLocalisationRow(localisationState, metadata.deckNameToken, deckName);
  return metadata;
}

/*
 * Nothing here removes an entry, on purpose. A build cannot tell a retired
 * division from one whose patch is off or that this run did not select, and
 * dropping an id hands the next build's divisions to decks saved against the old
 * ones. Clearing the store is the author's call, made by hand.
 */
...
function createDefaultPersistentSerializer(): PersistentStore['serializer'] {
  return {
    ...
    divisionIds: createEmptyRecord(),
    unitIds: createEmptyRecord(),
  };
}
...
function validatePersistentStore(value: unknown): PersistentStore {
  ...
  const nextDivisionNameToken = readTokenCounter(
    value.nextDivisionNameToken,
    'nextDivisionNameToken',
  );
  const nextDeckNameToken = readTokenCounter(value.nextDeckNameToken, 'nextDeckNameToken');
  if (!isRecord(value.divisions)) {
    throw new Error('`divisions` must be an object.');
  }

  const divisions = createEmptyRecord<PersistentDivisionMetadata>();
...
function validateSerializerIdMap(
  ...
  const result = createEmptyRecord<number>();
...
function readTokenCounter(value: unknown, label: string): number {
  if (
    typeof value !== 'number' ||
    !Number.isSafeInteger(value) ||
    value < 1 ||
    value > MAX_TOKEN_NUMBER + 1
  ) {
    throw new Error(`\`${label}\` must be a safe integer from 1 through ${MAX_TOKEN_NUMBER + 1}.`);
  }
  return value;
}

function assertTokenNumberAvailable(value: number, label: string): void {
  if (value > MAX_TOKEN_NUMBER) {
    throw new Error(
      `No safe ${label} localisation tokens remain in the eight-digit range 1-${MAX_TOKEN_NUMBER}.`,
    );
  }
}
