...
import { createDeckGenerationConfig, shouldIncludeEntityInVariant } from './generator/config.ts';
import {
...
export function generateDeckOutputs(input: DeckGenerationInput): DeckGenerationResult {
  ...
  const sortedDeckableEntities = [...deckableEntities].sort((left, right) =>
    left.name.localeCompare(right.name),
  );
  const maxActivationPoints = resolveDeckMaxActivationPointsFromCategories(
    config.deckSlotCount,
    deckableEntities.map((entity) => entity.factoryType),
  );
  const vanillaRules = parseDivisionRules(
  ...
  const divisions = contexts.flatMap((context, index) =>
    buildDivisionVariants({
      ...
      contextIndex: index,
      modTag: input.modTag,
      deckableEntities: sortedDeckableEntities,
      entityByName,
      transportMap,
      weaponDescriptors,
      ammunition,
